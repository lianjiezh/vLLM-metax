...
try:
    ...
    # /------------------------  Metax Modification -------------------------\
    cudnn_batch_prefill_with_kv_cache = None  # type: ignore
    # \------------------------- Metax Modification -------------------------/
...
aiter_triton_fp8_bmm = None  # type: ignore
...
class MLACommonBaseImpl(MLAAttentionImpl[A], Generic[A]):
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        if is_rocm_aiter_fp8bmm_enabled():
            ...
            for m in pre_compilation_list:
                ...
                aiter_triton_fp8_bmm(  # type: ignore
    ...
    def _v_up_proj(self, x: torch.Tensor, out: torch.Tensor):
        ...
        if is_rocm_aiter_fp8bmm_enabled():
            ...
            x = aiter_triton_fp8_bmm(  # type: ignore
                x, self.W_V, self.W_V_scale, group_size=128, transpose_bm=True
            )
...
class MLACommonImpl(MLACommonBaseImpl[M], Generic[M]):
    ...
    def _run_prefill_new_tokens_cudnn(
        ...
        output, lse = cudnn_batch_prefill_with_kv_cache(  # type: ignore
            q=q,
            k_cache=k,
            v_cache=v,
            scale=self.scale,
    ...
    def _run_prefill_context_chunk_cudnn(
        ...
        assert prefill.query_seq_lens is not None
        return cudnn_batch_prefill_with_kv_cache(  # type: ignore
            q=q,
            k_cache=k,
            v_cache=v,
            scale=self.scale,
    ...
    def forward(
        ...
        if has_decode:
            ...
            if is_rocm_aiter_fp8bmm_enabled():
                ...
                decode_ql_nope = aiter_triton_fp8_bmm(  # type: ignore
                    decode_q_nope,
                    self.W_K,
                    self.W_K_scale,
                    group_size=128,
